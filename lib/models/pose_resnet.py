...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        self.input_channels = kwargs['input_channels']\
            if 'input_channels' in kwargs else cfg.POSE_RESNET.INPUT_CHANNELS
        self.keep_scale = 'keep_scale' in kwargs and kwargs['keep_scale']

        super(PoseResNet, self).__init__()
        self.conv1 = nn.Conv2d(self.input_channels, 64, kernel_size=7, stride=2, padding=3, bias=False) \
            if not self.keep_scale else nn.Conv2d(self.input_channels, 64, kernel_size=7, stride=1, padding=3, bias=False)
        self.bn1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1) \
            if not self.keep_scale else lambda x: x 
...
resnet_spec = {10: (BasicBlock, [1, 1, 1, 1]),
               12: (BasicBlock, [2, 1, 1, 1]),
               14: (BasicBlock, [2, 2, 1, 1]),
               16: (BasicBlock, [2, 2, 2, 1]),
