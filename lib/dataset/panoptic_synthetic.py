...
from dataset.SynJointsDataset import SynJointsDataset
...
class PanopticSynthesize(SynJointsDataset):
    def __init__(self, cfg, image_set, is_train, dataset_root, transform=None, data_path=''):
        ...
        if osp.exists(self.db_file): # lazy loading
            ...
            if self.image_set == 'train':
                hmor = pickle.load(open(osp.join(self.dataset_root, 'train_cam.pkl'), 'rb'))
            elif self.image_set == 'validation':
                hmor = pickle.load(open(osp.join(self.dataset_root, 'valid_cam.pkl'), 'rb'))
            self.db = self._get_db(hmor)
